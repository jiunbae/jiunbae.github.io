...
resolvedDate: 2026-04-06T13:20:00+09:00
severity: major
status: resolved
affectedServices:
  - Gateway (CoreDNS)
...
timeline:
  ...
  - time: 2026-04-06T13:09:00+09:00
    status: monitoring
    message: "Registry LXC(130) 재생성 완료. pct create → Ansible docker-registry role 적용. 37개 이미지 리포지토리 정상 확인 (데이터는 /mnt/lxc-data/registry에 보존되어 있었음)."
  - time: 2026-04-06T13:15:00+09:00
    status: monitoring
    message: "Obsidian Sync LXC(150) 재생성 완료. CouchDB 3.5.1 정상 기동, obsidian 데이터베이스 보존 확인, CORS 설정 적용. TrueNAS(/mnt/backup/services/obsidian)에도 백업 데이터 존재 확인."
  - time: 2026-04-06T13:20:00+09:00
    status: resolved
    message: "전체 7개 LXC 정상 가동 확인. 모든 LXC에 protection:1 플래그 설정 완료. k3s-dev VM(200)도 정상."
---
...
| 데이터 손실 | 없음. Registry와 CouchDB 데이터는 `/mnt/lxc-data/`(LVM persistent storage)에 보존되어 있었음. TrueNAS에도 백업 존재 확인 |
...
- [x] 복구 가능한 LXC/VM 전체 시작 (110, 120, 140, VM 200)
- [x] 서비스 상태 전수 점검
- [x] 장애 원인 분석 (API 호출 출처 추적)
- [x] Registry LXC(130) 재생성 — `pct create` + Ansible docker-registry role. 37개 이미지 리포 정상
- [x] Obsidian Sync LXC(150) 재생성 — `pct create` + CouchDB 수동 배포. obsidian DB 보존 확인
- [x] 전체 LXC에 `protection: 1` 플래그 설정 (실수/자동화에 의한 삭제 방지)
- [x] TrueNAS 백업 데이터 존재 확인 (`/mnt/backup/services/obsidian`, `/mnt/backup/services/registry`)
- [x] Proxmox API 접근 제한 강화 — `terraform@pam`에서 Administrator 역할 제거, TerraformRole만 유지. `monitor@pam` + MonitorRole(읽기 전용) 생성
- [x] 파괴적 API 호출 실시간 감지 — `pve-api-watchdog.sh` 배포 (양쪽 Proxmox, 1분 주기 크론). shutdown/destroy/delete 감지 시 Telegram 인프라 봇으로 즉시 알림
- [ ] Claude Code 세션의 `--dangerously-skip-permissions` 사용 검토
- [ ] root@pam 비밀번호 변경

## 재발 방지 (Prevention)

### 1. Proxmox API 접근 제어 (적용 완료)
- `terraform@pam`: Administrator 역할 제거, TerraformRole(제한된 권한)만 유지
- `monitor@pam` + API 토큰 `monitor@pam!dashboard`: 읽기 전용 MonitorRole 생성
- 역할 구성: TerraformRole(VM 프로비저닝), MonitorRole(감사/읽기만), Administrator(root@pam만)

### 2. LXC `protection` 플래그 활성화 (적용 완료)
- 전체 7개 LXC(110~170)에 `protection: 1` 설정 — API로 삭제 시도 시 거부됨

### 3. 파괴적 API 호출 실시간 감지 (적용 완료)
- `pve-api-watchdog.sh` 양쪽 Proxmox 배포 (1분 주기 크론)
- pveproxy access log에서 shutdown/destroy/delete 패턴 감지
- Telegram 인프라 봇(@52237918)으로 즉시 알림

### 4. Claude Code 권한 관리 (미적용)
- `--dangerously-skip-permissions` 대신 적절한 권한 모드 사용 검토 필요
- 인프라 관리 세션의 Proxmox 크레덴셜 접근 범위 제한 필요
