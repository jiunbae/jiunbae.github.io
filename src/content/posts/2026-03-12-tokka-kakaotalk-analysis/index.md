...
Gemini 무료 티어는 키당 RPD 500입니다. 분석 파이프라인이 청크 분석 7회 + 합성 + 보충 + SNS로 ~10회 호출하면, 하루에 처리할 수 있는 분석 건수가 금방 바닥납니다.

5개 Google AI Studio 계정의 API 키를 round-robin으로 돌리도록 구현했습니다.

```python
_API_KEYS = _load_api_keys()  # GEMINI_API_KEYS env (쉼표 구분)
...
`itertools.cycle`과 `threading.Lock`으로 thread-safe하게 순환합니다. RPD 제한이 키당 1/5로 분산되어 일일 처리량이 5배 늘었습니다.

## 아직 남은 과제

하루 만에 배포했다는 건, 하루 만에 만든 기술 부채도 있다는 뜻입니다.

- **싱글 팟 문제**: nginx, uvicorn, email_worker가 하나의 팟에서 동작. 하나가 죽으면 전부 죽음
- **레이트 리미터 상태**: 인메모리 레이트 리미터는 팟 재시작 시 초기화됨
- **이메일 워커 분리**: IMAP 폴링이 메인 프로세스에서 백그라운드 스레드로 동작 중
- ~~**AI 리포트 파싱**: 그룹채팅 형식 미지원~~ → 해결됨
- ~~**그룹채팅 일부 참여자 누락**~~ → 보충 호출로 해결됨

새로 생긴 과제도 있습니다:

- **Sealed Secrets 자동화**: 현재 dev 시크릿은 수동 `kubectl` 주입. prod는 SealedSecret 사용 중이나 dev도 통일 필요
