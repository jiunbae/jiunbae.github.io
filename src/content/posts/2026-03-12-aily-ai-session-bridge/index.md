...
[aily](https://github.com/jiunbae/aily)는 tmux에서 돌아가는 AI 에이전트 세션과 Discord/Slack을 양방향으로 연결하는 오픈소스 브릿지입니다. 에이전트가 작업을 마치면 알림을 받고, 폰에서 답장하면 에이전트에 바로 전달됩니다.

### 이런 상황에서 유용합니다

- **에이전트가 여러 세션에서 동시에 돌아갈 때** — 어떤 세션이 끝났는지 터미널을 하나씩 확인할 필요 없이, Discord/Slack에서 한눈에 볼 수 있습니다.
- **에이전트가 질문하고 기다릴 때** — 폰 알림으로 바로 알 수 있고, 스레드에 답장만 하면 됩니다. SSH 접속 필요 없습니다.
- **원격 머신에서 에이전트를 돌릴 때** — 여러 SSH 호스트의 세션을 하나의 채널에서 관리할 수 있습니다.
- **자리를 비우거나 이동 중일 때** — 에이전트 상태를 폰에서 실시간으로 확인하고 제어할 수 있습니다.

### 주요 장점

| | |
|---|---|
| **에이전트 무관** | Claude Code, Codex, Gemini, OpenCode 등 tmux에서 돌아가는 에이전트라면 모두 지원 |
